# Register Feature Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Tambah halaman registrasi terbuka di `/register` agar pengguna bisa membuat akun sendiri via Supabase Auth.

**Architecture:** Halaman baru `Register.jsx` dengan layout split-panel identik Login.jsx. Route `/register` ditambah sebagai public route di App.jsx. Login.jsx mendapat link navigasi ke Register.

**Tech Stack:** React, React Router DOM, Supabase Auth, Framer Motion, Lucide React, Tailwind CSS

---

### Task 1: Buat `src/pages/Register.jsx`

**Files:**
- Create: `src/pages/Register.jsx`

- [ ] **Step 1: Buat file Register.jsx dengan layout split-panel dan form registrasi**
        ...
        <div className="w-full max-w-sm">
          ...
          {success ? (
            ...
            >
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">Akun berhasil dibuat!</h2>
                <p className="mt-2 text-sm text-gray-500 leading-relaxed">
                  Cek email kamu untuk konfirmasi akun sebelum login.
                </p>
              </div>
              <Link
                to="/login"
                className="btn-primary mt-2 inline-flex items-center gap-2 px-6 py-2.5 text-sm"
              >
                Pergi ke halaman login <ArrowRight size={15} />
              </Link>
            </motion.div>
          ) : (
            <>
              <div className="mb-8">
                ...
                <p className="mt-1 text-sm text-gray-500">Daftar untuk mengakses sistem klinik</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Konfirmasi Password */}
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-gray-700">Konfirmasi Password</label>
                  <div className="relative">
                    <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    <input
...
### Task 2: Tambah route `/register` di `src/App.jsx`

**Files:**
- Modify: `src/App.jsx`

- [ ] **Step 1: Tambah import Register dan route `/register`**

Ubah `src/App.jsx` — tambah import dan route baru:
...
### Task 3: Tambah link "Daftar" di `src/pages/Login.jsx`

**Files:**
- Modify: `src/pages/Login.jsx`

- [ ] **Step 1: Tambah import Link dari react-router-dom**

Di baris import Login.jsx, tambah `Link`:
```jsx
import { useNavigate, useLocation, Link } from 'react-router-dom'
```

- [ ] **Step 2: Tambah link daftar di bawah form, sebelum trust badges**

Tambahkan paragraf ini setelah penutup `</form>` dan sebelum `<div>` trust badges:
```jsx
<p className="mt-6 text-center text-sm text-gray-500">
  Belum punya akun?{' '}
...
```bash
git add src/pages/Register.jsx src/App.jsx src/pages/Login.jsx
git commit -m "feat: tambah halaman registrasi user"
```
